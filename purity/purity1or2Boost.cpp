...
pair<double, double> PurityFit(TH1D* chisoFake, TH1D* chisoPrompt, TH1D* chiso, string EBEE, string MRR2Bin){
	...
	if(status == 0){
		...
		sprintf(texte2, "JetAK8Masstag %s ", MRR2Bin.c_str());
	...
	CMS_lumi(C0, 2, 1);
	//C0->SetLogx();
	C0->SaveAs(Form("1or2Boost2016/Fraction_2016_Test_%s_%s.pdf", EBEE.c_str(), MRR2Bin.c_str()), "recreate");	
	
  TFile* f1 = new TFile("./1or2Boost2016/test.root", "update");
  C0->Write();
  result->Write();
  f1->Close();
  C0->Close();
...
pair<TH1D*, TH1D*> Purity2D(TFile* file, TString year){
	TH1::SetDefaultSumw2();
	//template - fake
	TH2D* htemp0_EB = (TH2D*)file->Get("1or2Boost_vs_PhotonCHIso_preslectphoptREBin/Data_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel"); 	//Barrel
	TH2D* htemp0_EE = (TH2D*)file->Get("1or2Boost_vs_PhotonCHIso_preslectphoptREBin/Data_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap"); 	//Endcap
	TH2D* htemp1_EB = (TH2D*)file->Get("1or2Boost_vs_PhotonCHIso_preslectphoptREBin/MC_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel");	//Barrel
	TH2D* htemp1_EE = (TH2D*)file->Get("1or2Boost_vs_PhotonCHIso_preslectphoptREBin/MC_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap");	//Barrel
	//template - prompt
	TH2D* htemp2_EB = (TH2D*)file->Get("1or2Boost_vs_PhotonCHIso_preslectphoptREBin/MC_PromptPhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel");	//Barrel
	TH2D* htemp2_EE = (TH2D*)file->Get("1or2Boost_vs_PhotonCHIso_preslectphoptREBin/MC_PromptPhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap");	//Endcap
	//fitting target
	TH2D* htemp3_EB = (TH2D*)file->Get("1or2Boost_vs_PhotonCHIso_preslectphoptREBin/Data_PhotonNoIso_CR_1PhoInv_Excl1Pho_"+year+"_Barrel");	//Barrel (Template)
	TH2D* htemp3_EE = (TH2D*)file->Get("1or2Boost_vs_PhotonCHIso_preslectphoptREBin/Data_PhotonNoIso_CR_1PhoInv_Excl1Pho_"+year+"_Endcap");	//Endcap (Template)
  ...
  cout << "end Fit" << endl;
	...
	TH1D* hist_R2_EB = new TH1D("hist_R2_EB", ";N_JetAK8 ;Photon purity", 2, 0, 2.5);
	TH1D* hist_R2_EE = new TH1D("hist_R2_EE", "", 2, 0, 2.5);
	TH1D* hist_R2_EB_MC = new TH1D("hist_R2_EB_MC", "", 2, 0, 2.5);
	TH1D* hist_R2_EE_MC = new TH1D("hist_R2_EE_MC", "", 2, 0, 2.5);
	...
	for(int i=1; i<=2; i++) hist_R2_EB->GetXaxis()->SetBinLabel(i, MRR2Bins[i-1].c_str());
	...
	leg->Draw("SAME");
	CMS_lumi(C1, 2, 1);
	C1->SaveAs(Form("1or2Boost2016/PurityResults.pdf"), "recreate");
...
pair<TH1D*, TH1D*> Fraction(TFile* f1, TString year){
	...
	fraction[0] = (TH2D*)f1->Get("DirectFraction_vs_1or2Boost/plus1d_Background_Prompt_Endcap_"+year+"_CR_1PhoInv");
    fraction[1] = (TH2D*)f1->Get("DirectFraction_vs_1or2Boost/plus1d_Background_Prompt_Barrel_"+year+"_CR_1PhoInv");
...
TGraphAsymmErrors* Correction(TH1D* purity_EB, TH1D* purity_EE, TH1D* frac_EB, TH1D* frac_EE, TFile* inputfile, TString year){
  ...
  bkg2D[0] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/Data_"+year+"_Endcap_CR_1PhoInv");
  bkg2D[1] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/Data_"+year+"_Barrel_CR_1PhoInv");
  bkg2D[2] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/DYToLL_"+year+"_CR_1PhoInv");
  bkg2D[3] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/Higgs_"+year+"_CR_1PhoInv");
  bkg2D[4] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/Multiboson_"+year+"_CR_1PhoInv");
  bkg2D[5] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/TT_powheg_pythia8_"+year+"_CR_1PhoInv");
  bkg2D[6] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/WToLNu_"+year+"_CR_1PhoInv");
  bkg2D[7] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/ZToNuNu_"+year+"_CR_1PhoInv");	
  bkg2D[8] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/Top_"+year+"_CR_1PhoInv");	
  bkg2D[9] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/Multijet_"+year+"_CR_1PhoInv"); //Exclude this from other BG
  bkg2D[10] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/GJets_"+year+"_CR_1PhoInv");	
  bkg2D[11] = (TH2D*)inputfile->Get("Counts_vs_1or2Boost/Syst_vs_1or2Boost/Data_"+year+"_CR_1PhoInv");	
...
// TGraphAsymmErrors *. -------------------------------------------------------
  string N_AK8[2] = {"=1", "=>2"};
  //TH1D* CF_MassTag_2016 = new TH1D("CF_MassTag_2016", "", 2, 0, 2.5);
/*
  for (int i = 1; i <=3; ++i)
  {
  	CF_MassTag_2016->SetBinContent(i,CF->GetBinContent(i));
  	CF_MassTag_2016->SetBinError(i,CF->GetBinError(i));
  }
*/  
  ...
  //CF->SetMaximum(40);
  //CF->SetMinimum(0);
  CF_GJets->SetMarkerStyle(21);
  CF_GJets->GetYaxis()->SetTitle("Correction Factor");
  CF_GJets->GetYaxis()->SetTitleOffset(0.9);
  CF_GJets->GetXaxis()->SetTitle("N_JetAK8");
  for(int i=1; i<=2; i++) CF_GJets->GetXaxis()->SetBinLabel(i, N_AK8[i-1].c_str());
 
  CF_GJets->Draw("AP");
  CMS_lumi(c1, 2, 1);

  c1->SaveAs(Form("1or2Boost2016/CorrectionFactor_fraction.pdf"), "recreate");
...
void results(){
	...
	//inputfile1 = TFile::Open("SinglePhotonData.root");
	//inputfile = TFile::Open("GJetsMC.root");
	inputfile = TFile::Open("Rootfiles/run_2022_05_15.root");

    TFile* f1  = new TFile("1or2Boost2016/CF_mass-tag_2016.root","recreate");
	//f1->mkdir("purity");
	//f1->mkdir("fraction");
	//f1->mkdir("correction");
...
*/
	cout << "purity calculation" << endl;
  	...
  	pair<TH1D*, TH1D*> purity_result = Purity2D(inputfile, "2016");
  ...
  TGraphAsymmErrors* CorFac = Correction(purity_MRR2_EB, purity_MRR2_EE, frac_EB, frac_EE, inputfile, "2016");
  ...
  C1->SaveAs("1or2Boost2016/Purity_EE.pdf", "recreate");
  C1->Close();

  C2->cd();
  purity_MRR2_EB->Draw("EP");
  C2->SaveAs("1or2Boost2016/Purity_EB.pdf", "recreate");
  C2->Close();

}

void purity1or2Boost(){
	results();
