...
pair<double, double> PurityFit(TH1D* chisoFake, TH1D* chisoPrompt, TH1D* chiso, string EBEE, string MRR2Bin, string year){
	TH1::SetDefaultSumw2();
	...
	if(status == 0){
		...
		sprintf(texte2, "NJet Bin: %s ", MRR2Bin.c_str());
	...
	CMS_lumi(C0, 2, 1);
	//C0->SetLogx();
	C0->SaveAs(Form("njet%s/Fraction_%s_Test_%s_%s.pdf", year.c_str(), year.c_str(), EBEE.c_str(), MRR2Bin.c_str()), "recreate");	

  	cout << purity << ", " << error << endl;
	return make_pair(purity,error);
}

pair<TH1D*, TH1D*> Purity2D(TFile* file, string year){
	TH1::SetDefaultSumw2();
	//template - fake
	TH2D* htemp0_EB = (TH2D*)file->Get(Form("NJetNoPhoBins_vs_PhotonCHIso_preslectphoptREBin/Data_FakePhoton_CR_1PhoInv_Excl1Pho1M_%s_Barrel",year.c_str())); 	//Barrel
	TH2D* htemp0_EE = (TH2D*)file->Get(Form("NJetNoPhoBins_vs_PhotonCHIso_preslectphoptREBin/Data_FakePhoton_CR_1PhoInv_Excl1Pho1M_%s_Endcap",year.c_str())); 	//Endcap
	TH2D* htemp1_EB = (TH2D*)file->Get(Form("NJetNoPhoBins_vs_PhotonCHIso_preslectphoptREBin/MC_FakePhoton_CR_1PhoInv_Excl1Pho1M_%s_Barrel",year.c_str()));	//Barrel
	TH2D* htemp1_EE = (TH2D*)file->Get(Form("NJetNoPhoBins_vs_PhotonCHIso_preslectphoptREBin/MC_FakePhoton_CR_1PhoInv_Excl1Pho1M_%s_Endcap",year.c_str()));	//Barrel
	//template - prompt
	TH2D* htemp2_EB = (TH2D*)file->Get(Form("NJetNoPhoBins_vs_PhotonCHIso_preslectphoptREBin/MC_PromptPhoton_CR_1PhoInv_Excl1Pho1M_%s_Barrel",year.c_str()));	//Barrel
	TH2D* htemp2_EE = (TH2D*)file->Get(Form("NJetNoPhoBins_vs_PhotonCHIso_preslectphoptREBin/MC_PromptPhoton_CR_1PhoInv_Excl1Pho1M_%s_Endcap",year.c_str()));	//Endcap
	//fitting target
	TH2D* htemp3_EB = (TH2D*)file->Get(Form("NJetNoPhoBins_vs_PhotonCHIso_preslectphoptREBin/Data_PhotonNoIso_CR_1PhoInv_Excl1Pho_%s_Barrel",year.c_str()));	//Barrel (Template)
	TH2D* htemp3_EE = (TH2D*)file->Get(Form("NJetNoPhoBins_vs_PhotonCHIso_preslectphoptREBin/Data_PhotonNoIso_CR_1PhoInv_Excl1Pho_%s_Endcap",year.c_str()));	//Endcap (Template)
	...
	string NJetBins[3] = {"2-4", "4-6", "6-20"};
	string EBEEs[2] = {"EB", "EE"};
  	cout << "Chiso Fit" << endl;
  	cout << "Size " << chisoFake_R2_EB.size() << endl;
	for(int i=0; i<chisoFake_R2_EB.size(); ++i){

		result = PurityFit(chisoFakeMC_R2_EB[i], chisoPrompt_R2_EB[i], chiso_R2_EB[i],EBEEs[0], NJetBins[i], year);
		purity_R2_EB_MC.push_back(result.first); error_R2_EB_MC.push_back(result.second);

		result = PurityFit(chisoFake_R2_EB[i], chisoPrompt_R2_EB[i], chiso_R2_EB[i], EBEEs[0], NJetBins[i], year);
		purity_R2_EB.push_back(result.first); error_R2_EB.push_back(result.second);

		result = PurityFit(chisoFakeMC_R2_EE[i], chisoPrompt_R2_EE[i], chiso_R2_EE[i], EBEEs[1], NJetBins[i], year);
		purity_R2_EE_MC.push_back(result.first); error_R2_EE_MC.push_back(result.second);

		result = PurityFit(chisoFake_R2_EE[i], chisoPrompt_R2_EE[i], chiso_R2_EE[i], EBEEs[1], NJetBins[i], year);
  ...
  cout << "end Fit" << endl;
	...
	TH1D* hist_R2_EB = new TH1D("hist_R2_EB", ";NJet ;Photon purity", 3, 0, 3.3);
	TH1D* hist_R2_EE = new TH1D("hist_R2_EE", "", 3, 0, 3.3);
	TH1D* hist_R2_EB_MC = new TH1D("hist_R2_EB_MC", "", 3, 0, 3.3);
	TH1D* hist_R2_EE_MC = new TH1D("hist_R2_EE_MC", "", 3, 0, 3.3);
	...
	for(int i=1; i<=5; i++) hist_R2_EB->GetXaxis()->SetBinLabel(i, NJetBins[i-1].c_str());
	...
	leg->Draw("SAME");
	CMS_lumi(C1, 2, 1);
	C1->SaveAs(Form("njet%s/PurityResults.pdf",year.c_str()), "recreate");

	return make_pair(hist_R2_EB, hist_R2_EE);
}

pair<TH1D*, TH1D*> Fraction(TFile* f1, string year){
	TH1::SetDefaultSumw2();
	TH2D* fraction[2];

	fraction[0] = (TH2D*)f1->Get(Form("DirectFraction_vs_NJetNoPhoBins/plus1d_Background_Prompt_Endcap_%s_CR_1PhoInv",year.c_str()));
    fraction[1] = (TH2D*)f1->Get(Form("DirectFraction_vs_NJetNoPhoBins/plus1d_Background_Prompt_Barrel_%s_CR_1PhoInv",year.c_str()));
...
*/
    ...
    TH1D* hist_EB = new TH1D("hist_EB", "", 3,0,3);
	TH1D* hist_EE = new TH1D("hist_EE", "", 3,0,3);
...
TGraphAsymmErrors* Correction(TH1D* purity_EB, TH1D* purity_EE, TH1D* frac_EB, TH1D* frac_EE, TFile* inputfile, string year){
  ...
  bkg2D[0] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/Data_%s_Endcap_CR_1PhoInv",year.c_str()));
  bkg2D[1] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/Data_%s_Barrel_CR_1PhoInv",year.c_str()));
  bkg2D[2] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/DYToLL_%s_CR_1PhoInv",year.c_str()));
  bkg2D[3] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/Higgs_%s_CR_1PhoInv",year.c_str()));
  bkg2D[4] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/Multiboson_%s_CR_1PhoInv",year.c_str()));
  bkg2D[5] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/TT_powheg_pythia8_%s_CR_1PhoInv",year.c_str()));
  bkg2D[6] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/WToLNu_%s_CR_1PhoInv",year.c_str()));
  bkg2D[7] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/ZToNuNu_%s_CR_1PhoInv",year.c_str()));	
  bkg2D[8] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/Top_%s_CR_1PhoInv",year.c_str()));	
  bkg2D[9] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/Multijet_%s_CR_1PhoInv",year.c_str())); //Exclude this from other BG
  bkg2D[10] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/GJets_%s_CR_1PhoInv",year.c_str()));	
  bkg2D[11] = (TH2D*)inputfile->Get(Form("Counts_vs_NJetNoPhoBins/Syst_vs_NJetNoPhoBins/Data_%s_CR_1PhoInv",year.c_str()));	
  ...
  TH1D * h_Nominal		 = new TH1D("Nominal","Nominal", binsize, bin);
  TH1D * h_Plus			 = new TH1D("Plus","Plus", binsize, bin);
  TH1D * h_Minus		 = new TH1D("Minus","Minus", binsize, bin);
  ...
  for (int i = 1; i <= bkg[0]->GetNbinsX(); ++i){
  	...
  	cout << "Nominal " << i << " counts  " << Nominal[i] << endl;
  	cout << "Plus " << i << " counts  " << Plus[i] << endl;
  	cout << "Minus " << i << " counts  " << Minus[i] << endl;
...
for(int i=1; i<= binsize;i++){
	...
	Cf_G_up[i-1] = std::sqrt(pow(abs(CF_UP->GetBinContent(i)-CF->GetBinContent(i)),2)+pow(CF->GetBinError(i),2));
	Cf_G_down[i-1] = std::sqrt(pow(abs(CF->GetBinContent(i))-CF_DW->GetBinContent(i),2)+pow(CF->GetBinError(i),2));
	cout << "Nominal-------------******* " << i << " counts  " << CF->GetBinContent(i) << endl;
	cout << "Up Value------------******* " << i << " counts  " << CF_UP->GetBinContent(i) << endl;
	cout << "Down Value------------***** " << i << " counts  " << CF_DW->GetBinContent(i) << endl;
	cout << "Up Error------------******* " << i << " counts  " << Cf_G_up[i-1] << endl;
	cout << "Down Error------------***** " << i << " counts  " << Cf_G_down[i-1] << endl;	
...
// TGraphAsymmErrors *. -------------------------------------------------------
  ...
  CF->GetXaxis()->SetTitle("NJet");
  CF->Draw("EP");
  CMS_lumi(c1, 2, 1);

  c1->SaveAs(Form("njet%s/CorrectionFactor_fraction.pdf", year.c_str()), "recreate");

  return CF_GJets;	
}

void results(string year){
	TCanvas* C1 = new TCanvas("C1", "", 700, 700);
	TCanvas* C2 = new TCanvas("C2", "", 700, 700);

    //inputfile = TFile::Open("RootFiles/run_2022_05_13.root");

	TFile *inputfile = TFile::Open("RootFiles/run_2022_05_13.root");

    TFile* f1  = new TFile(Form("njet%s/CF_GJetsNJET%s.root", year.c_str(), year.c_str()),"recreate");
	//f1->mkdir("purity");
	//f1->mkdir("fraction");
	//f1->mkdir("correction");

	pair<TH1D*, TH1D*> frac = Fraction(inputfile, year);
...
*/
	cout << "purity calculation" << endl;
  	...
  	pair<TH1D*, TH1D*> purity_result = Purity2D(inputfile, year);
  	TH1D* purity_MRR2_EB = std::get<0>(purity_result);
	TH1D* purity_MRR2_EE = std::get<1>(purity_result);
	
  //for(int i=1;i<=25;i++) cout << purity_EB->GetBinContent(i) << ", " << purity_EE->GetBinContent(i) << endl;

  TGraphAsymmErrors* CorFac = Correction(purity_MRR2_EB, purity_MRR2_EE, frac_EB, frac_EE, inputfile, year);
  ...
  C1->SaveAs(Form("njet%s/Purity_EE.pdf", year.c_str()), "recreate");
  C1->Close();

  C2->cd();
  purity_MRR2_EB->Draw("EP");
  C2->SaveAs(Form("njet%s/Purity_EB.pdf",year.c_str()), "recreate");
  C2->Close();
