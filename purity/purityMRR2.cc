...
pair<double, double> PurityFit(TH1D* chisoFake, TH1D* chisoPrompt, TH1D* chiso, string EBEE, string MRR2Bin){
	...
	CMS_lumi(C0, 2, 1);
	//C0->SetLogx();
	C0->SaveAs(Form("MRR2Template_2018/Fraction_2018_Test_%s_%s.pdf", EBEE.c_str(), MRR2Bin.c_str()), "recreate");	
	
  TFile* f1 = new TFile("./MRR2Template_2018/test.root", "update");
  C0->Write();
  result->Write();
  f1->Close();
  C0->Close();
...
pair<TH1D*, TH1D*> Purity2D(TFile* file, TString year){
	TH1::SetDefaultSumw2();
	//template - fake
	TH2D* htemp0_EB = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin/Data_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel"); 	//Barrel
	TH2D* htemp0_EE = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin/Data_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap"); 	//Endcap
	TH2D* htemp1_EB = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin/MC_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel");	//Barrel
	TH2D* htemp1_EE = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin/MC_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap");	//Barrel
	//template - prompt
	TH2D* htemp2_EB = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin/MC_PromptPhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel");	//Barrel
	TH2D* htemp2_EE = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin/MC_PromptPhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap");	//Endcap
	//fitting target
	TH2D* htemp3_EB = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin/Data_PhotonNoIso_CR_1PhoInv_Excl1Pho_"+year+"_Barrel");	//Barrel (Template)
	TH2D* htemp3_EE = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin/Data_PhotonNoIso_CR_1PhoInv_Excl1Pho_"+year+"_Endcap");	//Endcap (Template)
  ...
  cout << "end Fit" << endl;
	...
	for(int i=1; i<=3; i++) hist_R2_EB->GetXaxis()->SetBinLabel(i, MRR2Bins[i-1].c_str());
	...
	leg->Draw("SAME");
	CMS_lumi(C1, 2, 1);
	C1->SaveAs(Form("MRR2Template_2018/PurityResults.pdf"), "recreate");
...
pair<TH1D*, TH1D*> Fraction(TFile* f1, TString year){
	...
	fraction[0] = (TH2D*)f1->Get("DirectFraction_vs_MRR2NoPhoBins/plus1d_Background_Prompt_Endcap_"+year+"_CR_1PhoInv");
    fraction[1] = (TH2D*)f1->Get("DirectFraction_vs_MRR2NoPhoBins/plus1d_Background_Prompt_Barrel_"+year+"_CR_1PhoInv");
...
TGraphAsymmErrors* Correction(TH1D* purity_EB, TH1D* purity_EE, TH1D* frac_EB, TH1D* frac_EE, TFile* inputfile, TString year){
  ...
  bkg2D[0] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/Data_"+year+"_Endcap_CR_1PhoInv");
  bkg2D[1] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/Data_"+year+"_Barrel_CR_1PhoInv");
  bkg2D[2] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/DYToLL_"+year+"_CR_1PhoInv");
  bkg2D[3] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/Higgs_"+year+"_CR_1PhoInv");
  bkg2D[4] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/Multiboson_"+year+"_CR_1PhoInv");
  bkg2D[5] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/TT_powheg_pythia8_"+year+"_CR_1PhoInv");
  bkg2D[6] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/WToLNu_"+year+"_CR_1PhoInv");
  bkg2D[7] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/ZToNuNu_"+year+"_CR_1PhoInv");	
  bkg2D[8] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/Top_"+year+"_CR_1PhoInv");	
  bkg2D[9] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/Multijet_"+year+"_CR_1PhoInv"); //Exclude this from other BG
  bkg2D[10] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/GJets_"+year+"_CR_1PhoInv");	
  bkg2D[11] = (TH2D*)inputfile->Get("Counts_vs_MRR2NoPhoBins/Syst_vs_MRR2NoPhoBins/Data_"+year+"_CR_1PhoInv");	
  ...
  TH1D* bkg[12]; //1D MR*R2 data and background histograms
  ...
  for (int i = 1; i <= bkg[0]->GetNbinsX(); ++i){
  	...
  	//CF_G_up[i] = std::sqrt(pow(abs(Plus[i]-Nominal[i]),2));
  	//CF_G_up[i] = pow(CF_G_up[i],0.5);

  	//CF_G_down[i] = std::sqrt(pow(abs(Nominal[i]-Minus[i]),2));
  	//CF_G_down[i] = pow(CF_G_down[i],0.5);
  ...
  printf("k_G = %4.3f +- %4.3f (%4.3f / %4.3f) (2018) \n", DR, ek_G, DR_up, DR_down);
  printf("error_G = %4.3f, error_GJet %4.3f (2018) \n", ek_G, eG_GJ);
...
double Cf_G[binsize];
double Cf_G_up[binsize];
double Cf_G_up_err[binsize];
double Cf_G_down[binsize];
double Cf_G_down_err[binsize];
...
for(int i=1; i<= binsize;i++){
	...
	//Cf_G_up[i-1]=CF_UP->IntegralAndError(0, -1, Cf_G_up_err[i-1]);
	//Cf_G_down[i-1]=CF_DW->IntegralAndError(0, -1, Cf_G_down_err[i-1]);
	cout << "Up------------******* " << i << " counts  " << Cf_G_up_err[i-1] << endl;
	cout << "Down------------***** " << i << " counts  " << Cf_G_down_err[i-1] << endl;	
...
// TGraphAsymmErrors *. -------------------------------------------------------
  ...
  c1->SaveAs(Form("MRR2Template_2018/CorrectionFactor_fraction.pdf"), "recreate");
...
void results(){
	...
	inputfile = TFile::Open("RootFiles/run_2022_05_11.root");

    TFile* f1  = new TFile("MRR2Template_2018/CF_GJets2018.root","recreate");
	//f1->mkdir("purity");
	//f1->mkdir("fraction");
	//f1->mkdir("correction");

	pair<TH1D*, TH1D*> frac = Fraction(inputfile, "2018");
...
*/
	cout << "purity calculation" << endl;
  	...
  	pair<TH1D*, TH1D*> purity_results = Purity2D(inputfile, "2018");
  	TH1D* purity_MRR2_EB = std::get<0>(purity_results);
	TH1D* purity_MRR2_EE = std::get<1>(purity_results);
	
  //for(int i=1;i<=25;i++) cout << purity_EB->GetBinContent(i) << ", " << purity_EE->GetBinContent(i) << endl;

  TGraphAsymmErrors* CorFac = Correction(purity_MRR2_EB, purity_MRR2_EE, frac_EB, frac_EE, inputfile, "2018");
  ...
  C1->SaveAs("MRR2Template_2018/Purity_EE.pdf", "recreate");
  C1->Close();

  C2->cd();
  purity_MRR2_EB->Draw("EP");
  C2->SaveAs("MRR2Template_2018/Purity_EB.pdf", "recreate");
  C2->Close();
