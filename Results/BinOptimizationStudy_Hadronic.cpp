...
histo_1D[0][0] = new TH1D("First", ";Bin;Counts", 3000, 0, 3000);   // 8
  ...
  for (int i=0; i<8;i++){         //  Hadronic SR
    ...
    for (int x = 1; x <=htemp[0]->GetNbinsX() ; ++x)
    {
      ...
      histo_1D[i][0]->SetBinContent(x, htemp[0]->GetBinContent(x,1)); //Multijet
      histo_1D[i][1]->SetBinContent(x, htemp[1]->GetBinContent(x,1)); //Top
      histo_1D[i][2]->SetBinContent(x, htemp[2]->GetBinContent(x,1)); //WJet
      histo_1D[i][3]->SetBinContent(x, htemp[3]->GetBinContent(x,1)); //ZJet
      histo_1D[i][4]->SetBinContent(x, htemp[4]->GetBinContent(x,1)); //Multiboson
      histo_1D[i][5]->SetBinContent(x, htemp[5]->GetBinContent(x,1)); //T5ttcc
      histo_1D[i][6]->SetBinContent(x, htemp[6]->GetBinContent(x,1)); //T2bW
      histo_1D[i][7]->SetBinContent(x, htemp[7]->GetBinContent(x,1)); //T2bt
      ...
      histo_1D[i][0]->SetBinError(x, sqrt(error_1));
      histo_1D[i][1]->SetBinError(x, sqrt(error_2));
      histo_1D[i][2]->SetBinError(x, sqrt(error_3));
      histo_1D[i][3]->SetBinError(x, sqrt(error_4));
      histo_1D[i][4]->SetBinError(x, sqrt(error_5));
      histo_1D[i][5]->SetBinError(x, sqrt(error_6));
      histo_1D[i][6]->SetBinError(x, sqrt(error_7));
      histo_1D[i][7]->SetBinError(x, sqrt(error_8));
...
TH1D* hist_bins[8][8];  //// OPTIMIZATION STEP
...
for (int i = 0; i<8; ++i){
  ...
  c1->SaveAs("MRR2_Hadronic_OptedBins.png"); 
  c1->Close();

}

void BinOptimizationStudy_Hadronic(){
  UnrolledPlots();
}
