...
histo_1D[0][0] = new TH1D("First", ";Bin;Counts", 3000, 0, 3000);   // 8
  ...
  for (int i=20; i<29;i++){         //  Hadronic SR
    ...
    for (int x = 1; x <=htemp[0]->GetNbinsX() ; ++x)
    {
      ...
      histo_1D[i-20][0]->SetBinContent(x, htemp[0]->GetBinContent(x,1)); //Multijet
      histo_1D[i-20][1]->SetBinContent(x, htemp[1]->GetBinContent(x,1)); //Top
      histo_1D[i-20][2]->SetBinContent(x, htemp[2]->GetBinContent(x,1)); //WJet
      histo_1D[i-20][3]->SetBinContent(x, htemp[3]->GetBinContent(x,1)); //ZJet
      histo_1D[i-20][4]->SetBinContent(x, htemp[4]->GetBinContent(x,1)); //Multiboson
      histo_1D[i-20][5]->SetBinContent(x, htemp[5]->GetBinContent(x,1)); //T5ttcc
      histo_1D[i-20][6]->SetBinContent(x, htemp[6]->GetBinContent(x,1)); //T2bW
      histo_1D[i-20][7]->SetBinContent(x, htemp[7]->GetBinContent(x,1)); //T2bt
      ...
      histo_1D[i-20][0]->SetBinError(x, sqrt(error_1));
      histo_1D[i-20][1]->SetBinError(x, sqrt(error_2));
      histo_1D[i-20][2]->SetBinError(x, sqrt(error_3));
      histo_1D[i-20][3]->SetBinError(x, sqrt(error_4));
      histo_1D[i-20][4]->SetBinError(x, sqrt(error_5));
      histo_1D[i-20][5]->SetBinError(x, sqrt(error_6));
      histo_1D[i-20][6]->SetBinError(x, sqrt(error_7));
      histo_1D[i-20][7]->SetBinError(x, sqrt(error_8));
...
//Errors of the bins
Double_t ERR_[8][8][9]; // Err(i,j,k) --> i: Bin Counts, j: Process (Multijet, Top, etc.), k: SRs

//Integral of the bins
Double_t INT_[8][8][9]; // INT(i,j,k) --> i: Bin Counts, j: Process (Multijet, Top, etc.), k: SRs

int SR_1[9]  = {0, 200, 300, 400, 500, 600, 800, 1000, 3000};  // SR_Lep_1htop
int SR_2[9]  = {0, 200, 300, 350, 400, 500, 600, 1500, 3000};  // SR_Lep_V_b
int SR_3[9]  = {0, 200, 300, 350, 400, 500, 600, 700, 3000};  // SR_Lep_V_0b
int SR_4[9]  = {0, 200, 300, 400, 500, 600, 800, 1000, 3000};  // SR_Leptop_0htop
int SR_5[9]  = {0, 300, 500, 600, 700, 750, 800, 1000, 3000}; // SR_Leptop_1htop
int SR_6[9]  = {0, 200, 300, 400, 500, 600, 700, 1000, 3000};  // SR_Lepjet_0V_24j
int SR_7[9]  = {0, 200, 300, 400, 500, 600, 800, 1000, 3000}; // SR_Lepjet_0V_5j
int SR_8[9]  = {0, 200, 250, 400, 450, 600, 1000, 1500, 3000}; // SR_Lepjet_1V_24j
int SR_9[9]  = {0, 200, 250, 300, 350, 400, 600, 1000, 3000}; // SR_Lepjet_1V_5j

TH1D* hist_bins[9][8];  //// OPTIMIZATION STEP (72 1D plots with selected bin edges and bin numbers.)
...
histo_1[0] = new TH1D("First", ";Bin;Counts", 72, 0, 72);   // 8
...
string MRR2Bins_1[72] = {"0-200", "200-300", "300-400", "400-500", "500-600", "600-800", "800-1000", "1000-3000",   // SR_Lep_1htop
  ...
  TLine* lines_1[8];
  TLatex* SR_names_Higgs[8];

  for (int i = 20; i < 29; ++i){
      lines_1[i] = new TLine(((i-20)*8)+8,0,((i-20)*8)+8,1e4);
      lines_1[i]->SetLineStyle(2);
      lines_1[i]->Draw("SAME");

      SR_names_Higgs[i] = new TLatex(((i-20)*8)+4, 1e3, SigRegion[i].c_str());
  ...
  for(int i=1; i<=72; i++) hs1_1->GetXaxis()->SetBinLabel(i, MRR2Bins_1[i-1].c_str());
  ...
  TLatex* era_lat = new TLatex(72*0.95,3e5, text_1.c_str());
  ...
  c1->SaveAs("MRR2_Leptonic_OptedBins.png"); 
  c1->Close();

}

void BinOptimizationStudy_Leptonic(){
  UnrolledPlots();
}
