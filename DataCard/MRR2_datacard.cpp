...
void UnrolledPlots(string period, TString sample){
  ...
  string Systematic[] = {"", "_topptUp", "_topptDown",  "_isrUp", "_isrDown", "_pileupUp", "_pileupDown", "_L1PreFiringUp", "_L1PreFiringDown", "_alphasUp", "_alphasDown", "_scaleUp", "_scaleDown", "_lostlepUp", "_lostlepDown", "_triggerUp", "_triggerDown", "_jesUp", "_jesDown", "_jerUp", "_jerDown", "_metUp", "_metDown"}; 
...
cout<< " # of MassPoints 1 ------>  " <<MassPoints[851]<<endl;
  ...
  TH2D* Other_2D[sizeRegions];
  TH2D* TotMC_2D[sizeRegions];
  ...
  for (int i = 0; i < sizeRegions; ++i)
  {
      ...
      */
      ...
      Multijet_2D[i]   = (TH2D*)file0->Get(Form("Background/MRR2_bkg_%s", SigRegion[i].c_str()));
      WtoL_2D[i]       = (TH2D*)file1->Get(Form("Background/MRR2_bkg_%s", SigRegion[i].c_str()));
      ZtoNu_2D[i]      = (TH2D*)file2->Get(Form("Background/MRR2_bkg_%s", SigRegion[i].c_str()));
  ...
  TH1D* Other_1D[sizeRegions];
  TH1D* data_1D[sizeRegions];
  TH1D* TotMC_1D[sizeRegions];
...
for (int i = 0; i < sizeRegions; ++i){
  ...
  for(int u=0; u<sizeRegions; u++){
    ...
    TString name_Other      = Other_2D[u]->GetName();
    ...
    Other_1D[u]       = new TH1D(Form("Bkg_MultiBoson_%d",u), "", 138,0,138);
    TotMC_1D[u]       = new TH1D(Form("Bkg_TotMC_%d",u), "", 138,0,138);
    data_1D[u]        = new TH1D(Form("Data_%d",u), "", 138,0,138);
  ...
  for(int k=0;k<sizeRegions; k++){  
    ...
    for(int i=1;i<=Sig_2D[0][0]->GetNbinsY();i++){       // Number of Systematic Var. on Y axis :    23
      for(int j=1;j<=Sig_2D[0][0]->GetNbinsX();j++){     // Number of MR*R2 Bins on X-axis      :     6
        ...
        Other_1D[k]->SetBinContent(((i-1)*6)+j,Other_2D[k]->GetBinContent(j,i));  
        Other_1D[k]->SetBinError(((i-1)*6)+j,Other_2D[k]->GetBinError(j,i));

        TotMC_1D[k]->SetBinContent(((i-1)*6)+j,TotMC_2D[k]->GetBinContent(j,i));  
        TotMC_1D[k]->SetBinError(((i-1)*6)+j,TotMC_2D[k]->GetBinError(j,i));
...
// 1D plots unrolled for each signal region on the MR*R2 bins - 23 1D plots for each MC samples, 23*[masspoints] 1D plots for the signal.
  ...
  TH1F* hist_Other[2*n_var+1];
  TH1F* hist_TotMC[2*n_var+1];  
  //H1F* hist_MultiBoson[2*n_var+1];
...
for (int i=0; i<2*n_var+1; i++) { //23 number of systematic variations,
    ...
    hist_data[i]        = new TH1F(Form("MRR2_S_data%s",Systematic[i].c_str()) , "", 174,0,174); 
    hist_Multijet[i]    = new TH1F(Form("MRR2_S_bkg_Multijet%s",Systematic[i].c_str()) , "", 174,0,174);
    hist_WtoL[i]        = new TH1F(Form("MRR2_S_bkg_WJet%s",Systematic[i].c_str()) , "", 174,0,174);
    hist_ZtoNu[i]       = new TH1F(Form("MRR2_S_bkg_ZJet%s",Systematic[i].c_str()) , "", 174,0,174);
    hist_Top[i]         = new TH1F(Form("MRR2_S_bkg_Top%s",Systematic[i].c_str()) , "", 174,0,174);
    hist_Other[i]       = new TH1F(Form("MRR2_S_bkg_other%s",Systematic[i].c_str()) , "", 174,0,174);
    hist_TotMC[i]       = new TH1F(Form("MRR2_S_bkg_TotMC%s",Systematic[i].c_str()) , "", 174,0,174);
  ...
  for (int j = 1; j <=sizeRegions; ++j){   // Loop over # of Signal Regions         ( 29 )
    //for (int m = 0; m < sizeMassPoints; ++m){
      ...
      for (int k = 0; k < 6; ++k){          // Loop over # of MR*R2 bins             (  6 )
          ...
          hist_Other[i-1]->SetBinContent((((j-1)*6)+k),Other_1D[j-1]->GetBinContent(((i-1)*6)+k));
          hist_Other[i-1]->SetBinError(((j-1)*6)+k,Other_1D[j-1]->GetBinError(((i-1)*6)+k));

          hist_TotMC[i-1]->SetBinContent((((j-1)*6)+k),TotMC_1D[j-1]->GetBinContent(((i-1)*6)+k));
          hist_TotMC[i-1]->SetBinError(((j-1)*6)+k,TotMC_1D[j-1]->GetBinError(((i-1)*6)+k));
...
TDirectory* d4 = outFile->mkdir("Top");
TDirectory* d5 = outFile->mkdir("Other");
TDirectory* d6 = outFile->mkdir("Data");
TDirectory* d7 = outFile->mkdir("TotMC");
//TDirectory* d6 = outFile->mkdir("Signal");
   
  d1->cd();
  for (int i = 0; i <2*n_var+1; ++i) hist_Multijet[i]->Write();
  d1->Close();  
  ...
  d4->cd();
  for (int i = 0; i <2*n_var+1; ++i) hist_Top[i]->Write();
  d4->Close(); 

  d5->cd();
  for (int i = 0; i <2*n_var+1; ++i) hist_Other[i]->Write();
  d5->Close();  

  d6->cd();
  for (int i = 0; i <2*n_var+1; ++i) hist_data[i]->Write();  
  d6->Close();

  d7->cd();
  for (int i = 0; i <2*n_var+1; ++i) hist_TotMC[i]->Write();  
  d7->Close();
